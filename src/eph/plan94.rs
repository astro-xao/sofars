use crate::consts::{D2PI, DAS2R, DJ00, DJM};
use crate::vm::anpm;
...
///
///  Returned:
///     pv     [[f64; 3]; 2] planet p,v (heliocentric, J2000.0, au,au/d)
///
...
///
pub fn plan94(date1: f64, date2: f64, np: i32) -> Result<([[f64; 3]; 2], i32), i32> {
    /* Gaussian constant */
    ...
    /* Mean longitude (degree and arcsecond) */
    const DLM_COEFFS: [[f64; 3]; 8] = [
    ...
    /* Longitude of the perihelion (degree and arcsecond) */
    const PI_COEFFS: [[f64; 3]; 8] = [
    ...
    /* Inclination (degree and arcsecond) */
    const DINC_COEFFS: [[f64; 3]; 8] = [
    ...
    /* Longitude of the ascending node (degree and arcsecond) */
    const OMEGA_COEFFS: [[f64; 3]; 8] = [
    ...
    let p_idx = (np - 1) as usize;

    /* Time: Julian millennia since J2000.0. */
    let t = ((date1 - DJ00) + date2) / DJM;

    /* OK status unless remote date. */
    jstat = if t.abs() <= 1.0 { 0 } else { 1 };

    /* Compute the mean elements. */
    let mut da = A_COEFFS[p_idx][0] + (A_COEFFS[p_idx][1] + A_COEFFS[p_idx][2] * t) * t;
    let mut dl = (3600.0 * DLM_COEFFS[p_idx][0] + (DLM_COEFFS[p_idx][1] + DLM_COEFFS[p_idx][2] * t) * t) * DAS2R;
    let de = E_COEFFS[p_idx][0] + (E_COEFFS[p_idx][1] + E_COEFFS[p_idx][2] * t) * t;
    let dp = anpm((3600.0 * PI_COEFFS[p_idx][0] + (PI_COEFFS[p_idx][1] + PI_COEFFS[p_idx][2] * t) * t) * DAS2R);
    let di = (3600.0 * DINC_COEFFS[p_idx][0] + (DINC_COEFFS[p_idx][1] + DINC_COEFFS[p_idx][2] * t) * t) * DAS2R;
    let dom = anpm((3600.0 * OMEGA_COEFFS[p_idx][0] + (OMEGA_COEFFS[p_idx][1] + OMEGA_COEFFS[p_idx][2] * t) * t) * DAS2R);

    /* Apply the trigonometric terms. */
    let dmu = 0.35953620 * t;
    for k in 0..8 {
        let arga = KP[p_idx][k] * dmu;
        let argl = KQ[p_idx][k] * dmu;
        da += (CA[p_idx][k] * arga.cos() + SA[p_idx][k] * arga.sin()) * 1e-7;
        dl += (CL[p_idx][k] * argl.cos() + SL[p_idx][k] * argl.sin()) * 1e-7;
    }
    let arga = KP[p_idx][8] * dmu;
    da += t * (CA[p_idx][8] * arga.cos() + SA[p_idx][8] * arga.sin()) * 1e-7;
    for k in 8..10 {
        let argl = KQ[p_idx][k] * dmu;
        dl += t * (CL[p_idx][k] * argl.cos() + SL[p_idx][k] * argl.sin()) * 1e-7;
    }
    dl = dl.rem_euclid(D2PI);
    ...
    let mut k = 0;
    let mut dae: f64 = 1.0;
    while k < KMAX && dae.abs() > 1e-12 {
        dae = (am - ae + de * ae.sin()) / (1.0 - de * ae.cos());
        ae += dae;
        k += 1;
        if k == KMAX - 1 {
            jstat = 2;
    ...
    let v = GK * ((1.0 + 1.0 / AMAS[p_idx]) / (da * da * da)).sqrt();
    ...
    let vx = v * ((-1.0 + 2.0 * xp * xp) * xms + xpxq2 * xmc);
    let vy = v * ((1.0 - 2.0 * xq * xq) * xmc - xpxq2 * xms);
    let vz = v * (2.0 * ci2 * (xp * xms + xq * xmc));

    /* Rotate to equatorial. */
    pv[1][0] = vx;
    pv[1][1] = vy * COSEPS - vz * SINEPS;
    pv[1][2] = vy * SINEPS + vz * COSEPS;
